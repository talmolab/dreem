...
def file_to_docs_url(file_path: str, base_url: str) -> dict | None:
    ...
    Args:
        file_path: Path to the changed file (e.g., "docs/usage.md")
        base_url: Base URL for the preview (e.g., "https://dreem.sleap.ai/pr/123/")
...
def categorize_changes(files: list[str], base_url: str) -> dict:
    ...
    for file_path in files:
        # Check for API-affecting changes (dreem package)
        if file_path.startswith("dreem/"):
            api_affected = True
            continue
